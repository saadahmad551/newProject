...
// Define form element
    const addFeatureForm = document.getElementById('re_add_nearby_form');
...
// Submit button handler
    const FeatureSubmitButton = document.getElementById('re_add_nearby_submit');
...
// Delete Floor category
{
    function deleteNearby(id) {
        Swal.fire({
            html: `Are you sure you want to delete this nearby?`,
            icon: "question",
            buttonsStyling: false,
            ...
            if (result.isConfirmed) {
                window.location.replace("nearby/delete/" + id);
            }
        });
    }
}
...
{
// Define form element
    const editFeatureForm = document.getElementById('re_edit_nearby_form');
...
// Submit button handler
    const editFeatureSubmitButton = document.getElementById('re_edit_nearby_submit');
    ...
    function editNearby(id) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $("#name_edit").val(data.nearby.name);
                $("#edit_icon").val(data.nearby.icon);
                $("#id").val(data.nearby.id);

                if (data.nearby.category) {
                    // Create the options
                    ...
                    if (data.nearby.category === "Main Features") {
                        option2.attr("selected", "selected");
                    } else if (data.nearby.category === "Plot Features") {
                        option3.attr("selected", "selected");
                    }else if (data.nearby.category === "Community Features") {
                        option4.attr("selected", "selected");
                    }else if (data.nearby.category === "Health and Recreational") {
                        option5.attr("selected", "selected");
                    }else if (data.nearby.category === "Communication and Internet") {
                ...
                $("#re_edit_nearby_modal").modal("show");
                blockUI.release();
            },
            error: function () {
                $("#re_edit_nearby_modal").modal("hide");
                blockUI.release();
            }
        });
    }
